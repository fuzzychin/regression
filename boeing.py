...
model = smf.ols(formula="distance ~ duration + no_pasg + speed_ground + height + pitch", data=boeing_df)
results = model.fit()
print(results.summary())

# Build revised model with distance as dependant variable and significant variables as independent variables
revised_model = smf.ols(formula="distance ~ speed_ground + height", data=boeing_df).fit()
